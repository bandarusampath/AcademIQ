				...
				if ( in_array( $role, array_keys( WLSM_M_Role::get_roles() ) ) ) {
					...
					if ( WLSM_M_Role::check_permission( array( 'manage_exams' ), $permissions ) ) {
						...
						if ( WLSM_M_Role::check_permission( array( 'manage_exams' ), $permissions ) ) {
							...
							$school_staff_exam_admit_cards_submenu = add_submenu_page( WLSM_MENU_STAFF_EXAMINATION, esc_html__( 'Admin Cards Print', 'school-management' ), esc_html__( 'Admit Cards Print', 'school-management' ), 'read', WLSM_MENU_STAFF_EXAM_ADMIT_CARDS_BULK_PRINT, array( 'WLSM_Menu', 'school_staff_exam_admit_cards_bulk_print' ) );
							add_action( 'admin_print_styles-' . $school_staff_exam_admit_cards_submenu, array( 'WLSM_Menu', 'menu_page_assets' ) );
